...
#define visual_time_per_step 200      // time it takes to complete every step of the visual range [miliseconds]
...
void interrupt Interrupciones ()
{
    ...
    */
    ...
    if((1 == TMR0IF) && (1 == TMR0IE)){              //Interrupt each millisecond
        TMR0IF = 0;
        TMR0 = 6;
        if(0 == rotation_section){
            if(visual_degree_counter++ >= visual_time_per_step){
                ...
                RC0 = 1;                            //turn off error and on success
                RC1 = 0;
                //------------------------------------------------------------------
...
void GPIO_ini()
{
    TRISC = 0xBC;  //RC0,RC1,Tx as outputs
    RC0 = 1;       //Starts off
    RC1 = 1;       //Starts off
